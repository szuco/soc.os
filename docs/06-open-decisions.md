...
| # | Prio | Entscheidung | Bemerkung |
|---|---|---|---|
| 57 | **P0** | **Der LM393 kann die obere Trip-Schwelle vermutlich gar nicht sehen.** Er wird aus `3V3_SYS` versorgt (Pin 8). Der klassische LM393 ist für einen Gleichtakt-Eingangsbereich von 0 bis **V+ − 1,5 V** spezifiziert, bei 3,3 V also **bis 1,8 V**. `V_TRIP_HI` liegt bei **2,752 V**, der Sensepegel im Trip-Fall ebenso. Die obere Hälfte des Fensterkomparators arbeitet damit außerhalb ihres zulässigen Bereichs — der Hardware-Trip würde nur in **einer** Stromrichtung funktionieren. Der Fehler ist unabhängig von der Strommessung und war vorher genauso da: Auch die alte Schwelle lag bei 2,752 V | **Drei Wege, am 17.08.2026 auf der gerouteten Platine nachgemessen.** Die Komparatoren sitzen bei (8,0 / 10,7) und (19,6 / 7,6), also rechts. **(a) `5V_SYS`:** Gleichtaktbereich 0–3,5 V, deckt 2,752 V knapp ab. Das Netz liegt aber komplett links (x = −23 … −8), nächster Punkt **27,3 bzw. 36,0 mm** entfernt — zwei lange Bahnen quer über eine Platine mit 985 Segmenten. **(b) `12V_GATE`:** nur **7,1 bzw. 3,2 mm** entfernt, Gleichtaktbereich dann 0–10,5 V, also reichlich. Aber es ist die Gate-Treiber-Schiene aus dem L78L12 — die lauteste Kleinspannung auf der Platine, und der Komparator hat keinen eigenen Abblockkondensator. **(c) Komparator mit Rail-to-Rail-Eingang und Open-Drain-Ausgang**, z. B. TLV3702 oder TLV1702, SOIC-8 mit dem Standard-Pinout eines dualen Komparators — also **derselbe Footprint, dieselben Netze, keine einzige Leiterbahn geändert**. Bleibt an 3,3 V und damit auf derselben Schiene wie die INA240-Referenz, was die ratiometrische Kopplung der Schwelle erhält. **Empfehlung: (c).** Der tatsächlich bestellte Typ ist gegen sein Datenblatt zu prüfen; der Schaltplan trägt bisher nur „LM393" ohne Herstellernummer |
...
| 59 | P2 | **Der Komparator sieht das ungefilterte Sense-Signal.** Der RC-Filter (1 k / 1 n, 1 µs) sitzt nur im Zweig zum ESP32-ADC; `Mx_ISNS` geht direkt auf beide LM393-Eingänge. Zusammen mit dem Latch genügt ein einzelner Störimpuls aus dem PWM-Schalten, um die Endstufe dauerhaft abzuschalten | Erst am realen Aufbau zu beurteilen — vielleicht reicht die Bandbreitenbegrenzung des INA240 (400 kHz). Falls nicht: ein zweiter RC von etwa 100 ns vor die Komparatoreingänge. Zwei Bauteile je Kanal, Platz auf der Platine vorausgesetzt |
