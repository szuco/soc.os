...
| # | Prio | Entscheidung | Bemerkung |
|---|---|---|---|
| 57 | **P0** | **Der LM393 kann die obere Trip-Schwelle vermutlich gar nicht sehen.** Er wird aus `3V3_SYS` versorgt (Pin 8). Der klassische LM393 ist für einen Gleichtakt-Eingangsbereich von 0 bis **V+ − 1,5 V** spezifiziert, bei 3,3 V also **bis 1,8 V**. `V_TRIP_HI` liegt bei **2,752 V**, der Sensepegel im Trip-Fall ebenso. Die obere Hälfte des Fensterkomparators arbeitet damit außerhalb ihres zulässigen Bereichs — der Hardware-Trip würde nur in **einer** Stromrichtung funktionieren. Der Fehler ist unabhängig von der Strommessung und war vorher genauso da: Auch die alte Schwelle lag bei 2,752 V | Zwei Wege. **(a) LM393 aus `5V_SYS` versorgen** — Gleichtaktbereich dann 0–3,5 V, deckt 2,752 V ab. Die Ausgänge sind Open-Collector, der Pull-Up bleibt an 3,3 V, die Logik sieht weiter 3,3-V-Pegel. Der Teiler bleibt an `3V3_SYS`, damit er ratiometrisch zur INA240-Referenz bleibt. Kostet **ein Netz auf der bereits gerouteten Bottom-Platine**. **(b) Komparator mit Rail-to-Rail-Eingang und Open-Drain-Ausgang** einsetzen, z. B. TLV3702 (SOIC-8, dual). Kein Layout betroffen, aber ein neues Bauteil und langsamer. **Vorschlag: (a)** — ein Netz umrouten ist überschaubarer als ein ungeprüfter Bauteiltausch. Vorher am Datenblatt des tatsächlich bestellten Typs bestätigen; der Schaltplan trägt nur „LM393" ohne Herstellernummer |
